"""
AI Drive Game - Clean Simple Version
"""
...
def get_game_state_text(car_lane, obstacles):
    ...
    if not threats:
        return f"Car in lane {car_lane}. Road CLEAR."

    threats.sort(key=lambda t: -t[2])
    blocked = set(t[0] for t in threats)
    clear = [i for i in range(1, 5) if i not in blocked]

    msg = f"Car lane {car_lane}. "
    for lane_num, prox, _ in threats:
        msg += f"Lane {lane_num} has obstacle. "
    if clear:
        msg += f"Clear lanes: {clear}."
    return msg
